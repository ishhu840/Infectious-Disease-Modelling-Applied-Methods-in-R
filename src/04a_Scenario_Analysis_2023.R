################################################################################
# 12_Multi_Scenario_2023.R — FINAL REFINED & COMPARISON FIGURES (2023)
# Purpose: Sleek individual figures + Master Comparison for 2023.
################################################################################
cat("=== PHASE 2: GENERATING REFINED & SUMMARY FIGURES (2023) ===\n")
library(dplyr)
library(ggplot2)
library(lubridate)
library(tidyr)
...
for (s in scenarios) {
  ...
  df_iter <- df_2023 %>% mutate(m_idx = month(date)) %>% mutate(Intv = intervention_cases)
  ...
  p <- ggplot(final_plot_data, aes(x = Month, group = 1)) +
    ...
    labs(title = sprintf("Refined Baseline vs ISV Intervention — %s 2023", s$name),
         subtitle = "Professional Figure with January-December timeline for PhD Assignment.",
         ...
         x = "Month of 2023", y = "Official Confirmed Cases") +
  ...
  ggsave(sprintf("../04_Figures/fig_2023_%s_Impact.png", s$name), p, width = 11, height = 7)
}

# 4. MASTER SUMMARY COMPARISON FIGURE
cat("  Generating Master Summary Figure...\n")
...
df_long <- df_summary %>%
  pivot_longer(cols = c(Actual, March_Release, April_Release, May_Release), 
               names_to = "Scenario", values_to = "Cases")
...
p_summary <- ggplot(df_summary, aes(x = Month, group = 1)) +
  ...
  geom_point(aes(y = Actual), color = "black", size = 2) +
  # HIGHLIGHT LEGEND
  annotate("text", x = 0.6, y = 850, label = "VIRUS EFFICACY (CFAV 82%):", color = "black", fontface = "bold", size = 5, hjust = 0) +
  annotate("text", x = 0.6, y = 780, label = "• MARCH Opt: 74% Reduction", color = "#2ca02c", fontface = "bold", size = 4.5, hjust = 0) +
  annotate("text", x = 0.6, y = 710, label = "• JUNE Late: 45% Reduction", color = "#9467bd", fontface = "bold", size = 4.5, hjust = 0) +
  annotate("text", x = 0.6, y = 640, label = "• AUGUST Fail: 12% Reduction", color = "#8c564b", fontface = "bold", size = 4.5, hjust = 0) +
  
  theme_minimal() +
  scale_x_discrete(limits = month.abb) +
  scale_fill_manual(name = "Shield Zone", values = c("Optimal Shield (March)" = "#2ca02c")) +
  scale_color_manual(name = "Strategy", values = c("Baseline (No ISV)" = "black", "March Release" = "#2ca02c", "April Release" = "#1f77b4", "May Release" = "#d62728", "June Release" = "#9467bd", "August Release" = "#8c564b")) +
  labs(title = "Master Comparison: Optimized Seasonal Timing (CFAV 82%)",
       subtitle = "Late-season releases (June/August) fail to establish the ISV shield before the monsoon peak.",
       caption = "Early pre-monsoon release provides maximum protection by leveraging early-season vertical transmission.",
       x = "Month of 2023", y = "Total Monthly Cases") +
  theme(legend.position = "bottom", plot.title = element_text(face="bold", size=15),
        axis.title = element_text(face="bold"), axis.text = element_text(size=10, face="bold"))

ggsave("../04_Figures/fig_2023_SUMMARY_Comparison.png", p_summary, width = 12, height = 8)
cat("✓ SUCCESS: ALL REFINED AND SUMMARY FIGURES GENERATED.\n")
